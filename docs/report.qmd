...
This project integrates two main data sources into a single, ZCTA-level dataset of police and neighborhood characteristics in New York City: the source data are the 2023 NYPD Arrest Dataset, which comprises of discrete arrest events and include ZIP codes of arrests, offense type (felony, misdemeanor, and violation), along with event-level attributes; analysis aggregates events to the ZIP-code level to calculate total arrests and arrest intensity.
The second source is the 2019–2023 American Community Survey (ACS) 5-Year Estimates, which provide demographic and socioeconomic variables at the ZCTA level. Key demographic measures include the percentage of Black, Hispanic, White, and Asian residents and socioeconomic measures include median household income, poverty rate, and total population size. These variables are used both as controls and predictors in the statistical models and allow per-capita normalization of arrest counts.
...
This exploratory phase provides a descriptive grounding for the modeling and fairness analysis in the consequent sections. 
...
The five ZIP codes with the highest arrest intensities, along with the percentage of the demographic composition of that region, are shown in @tbl-high-arrest. Most of these zipcodes fall in historically disadvantaged areas: the South Bronx (10451, 10454), East Harlem (10035), and parts of Staten Island’s North Shore (10301). 3 out of the 5 zip codes shown above demonstrate high proportions of Black and Hispanic residents. This correlated relationship (though not causal) may suggest uneven policing in these areas. 
...
| ZCTA      | Arrests per 1,000 | % Black | % Hispanic | % White | % Asian | Median Income |
|-----------|-------------------|---------|------------|---------|---------|----------------|
| **11040** | 0.61              | 1%      | 11%        | 39%     | 44%     | $153,511       |
| **11001** | 1.17              | 6%      | 15%        | 51%     | 24%     | $144,755       |
| **10128** | 2.95              | 5%      | 13%        | 67%     | 10%     | $148,705       |
| **10075** | 3.02              | 1%      | 5%         | 79%     | 10%     | $146,556       |
| **11364** | 3.13              | 2%      | 15%        | 29%     | 50%     | $101,222       |
: Lowest-Arrest ZIP Codes in New York City (Top 5)
{#tbl-low-arrest}
:::

On the converse, the lowest-arrest ZIP codes in New York City form a clear and distinct group: they are among the wealthiest, least-policed neighborhoods in the city. Median household incomes in these areas range from roughly $100k to $158k and represent a highly skewed racial composition. These ZIP codes are predominantly White and Asian with very small Black and Hispanic populations (often below 5%). Arrest intensities in these neighborhoods are extraordinarily low ranging from 0.6 to 3.6 arrests per 1,000 residents; a metric that is substantially lower than the citywide mean of roughly 23 arrests per 1,000. 

Furthermore, these ZIP codes also share similar built-in environment characteristics by being considered as suburban-like. These areas have had stable residential patterns and historically receive lower levels of proactive policing, reflecting actual differences in crime reporting environments and engraved geographical inequities. 
...
Secondly and equally as notable, higher-income areas exhibit substantially lower arrest intensity. Median income is negatively correlated with arrest rate (-0.24), confirming geographic inequalities where neighborhoods with higher incomes and lower socioeconomic precarity see significantly less policing exposure. 
...
Before presenting the regression models, it is important to acknowledge that several of the demographic and socioeconomic predictors are highly correlated with one another. Racial composition, poverty rate, and median income exhibit strong pairwise correlations and produce large condition numbers in Models 2 and 3, indicating substantial multicollinearity.

Multicollinearity does not bias OLS coefficient estimates, but it does increase uncertainty and makes it more difficult to interpret the unique contribution of each predictor. For this reason, coefficient sizes should be interpreted as describing overall directional associations rather than isolated causal effects. Despite this limitation, the consistency of racial coefficients across specifications—remaining large and statistically significant even as socioeconomic variables are added—strengthens the conclusion that racial composition is independently associated with arrest intensity.

::: center
| Predictor         | Model 1 | Model 2 | Model 3 |
|------------------|---------|---------|---------|
| **pct_black**     | 13.17** | 13.58**  | 12.06 |
| **pct_hispanic**  | 30.05 | 30.55 | 26.34 |
| **median_income** | — | 0.000040 | 0.000069 |
| **below_poverty** | — | 0.00020  | 0.00090 |
| **log_pop**       | — | — | -11.53 |
| **R²**            | 0.135 | 0.141 | 0.176 |
: OLS Regression Coefficients Predicting Arrest Intensity  
{#tbl-ols-summary}
:::

Please refer to @tbl-ols-summary for analyses in consequent sections. 
...
In structural terms, these ZIP codes form the core of the city's arrest hotspot geography, further supported by the additional geospatial analysis to follow. 
...
The base arrest-intensity map presents a distinct spatial pattern, rather than random geographic variation (See @fig-baseline-map). High-arrest ZIP codes cluster in the Bronx, Northern Manhattan, and central and eastern Brooklyn - areas containing some of the city's highest proportions of Black and Hispanic residents. Low-arrest ZIP codes, on the other hand, appear in predominantly White, high-income neighborhoods including the Upper East Side, Battery Park/Tribeca, and sections of Staten Island and Queens. This simple initial visualization suggests that spatial patterns in policing intensity correspond closely to New York City's broader racial and socioeconomic geography.

![Spatial distribution of arrest intensity across New York City ZIP Code Tabulation Areas (ZCTAs), categorized into six levels based on arrests per 1,000 residents](../outputs/nyc_arrest_intensity_baseline.png){#fig-baseline-map width="100%"}


To place these findings in context and elaborate on them, maps of several of the key demographic variables used in the regression models—percent Black, percent Hispanic, median income, and poverty rate—were created. The racial composition maps reveal intense clustering of Black residents in Southeast Queens, Northern Manhattan, and the Bronx, while Hispanic residents concentrate in Northern Manhattan, the South Bronx, and parts of Brooklyn. 

![Spatial distribution of poverty across New York City ZIP Code Tabulation Areas (ZCTAs)](../outputs/nyc_poverty_map.png){#fig-poverty-map width="75%"}

The poverty map reveals high poverty counts concentrated in the Bronx and Central Brooklyn  (See Figure @fig-poverty-map). When viewed in conjunction with the arrest-intensity map (@fig-baseline-map), these demographic trends reveal significant spatial overlap: neighborhoods with greater Black and Hispanic populations and higher poverty counts correspond to areas of highest arrest intensity. This spatial congruence visually displays the regression results, which show that racial composition remains a strong predictor of arrest intensity even when income and poverty are controlled.


## Spatial Smoothing and Z-Score Standardization
...
## Arrest Data Reflect Enforcement, Not Crime

A central limitation of this study is that arrest data measure the behavior of the police, not the underlying level of unlawful activity in a neighborhood. Arrest intensity therefore reflects patterns of enforcement rather than serving as a neutral indicator of crime prevalence. This distinction is especially important in racialized and economically marginalized areas where historical patterns of over-policing may inflate arrest counts independently of underlying wrongdoing. For this reason, the results cannot be interpreted as demonstrating that communities with higher arrest intensity have higher rates of criminal behavior, but instead speak directly to enforcement practices and the structural conditions shaping them.

## ZIP Codes Are Imperfect Proxies for Neighborhoods

Another limitation arises from the use of ZIP Code Tabulation Areas as the primary geographic unit of analysis. ZCTAs are administrative constructs designed for mail delivery, not for capturing socially or economically meaningful neighborhood boundaries. They vary widely in physical size and population, can combine multiple distinct communities, and often fail to align with police precinct boundaries. As a result, some spatial patterns may be artifacts of the geographic unit itself rather than true reflections of community-level dynamics.

## Multicollinearity Between Race and Socioeconomic Variables 

A further methodological limitation stems from strong multicollinearity among racial composition, poverty levels, and median income in New York City. As mentioned earlier in the paper, these variables are deeply intertwined due to long-standing patterns of segregration and economic stratification. Though it does not directly bias coefficient estimates in our analysis, the overlapping nature of these predictors reduces the precision of causal interpretation. 
